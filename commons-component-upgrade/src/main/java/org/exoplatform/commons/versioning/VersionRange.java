...
public class VersionRange {
    private final ProductVersion recommendedVersion;

    private final List<Restriction> restrictions;

    private VersionRange( ProductVersion recommendedVersion, List<Restriction> restrictions )
    {
        this.recommendedVersion = recommendedVersion;
        this.restrictions = restrictions;
    }

    public ProductVersion getRecommendedVersion()
    {
    ...
    public static VersionRange createFromVersionSpec( String spec ) throws InvalidVersionSpecificationException
    {
        ...
        ProductVersion upperBound = null;
        ProductVersion lowerBound = null;
        ...
        if ( process.length() > 0 )
        {
            ...
            {
                version = new PlatformProductVersion( process );
    ...
    private static Restriction parseRestriction( String spec ) throws InvalidVersionSpecificationException {
        ...
        if ( index < 0 )
        {
            ...
            ProductVersion version = new PlatformProductVersion( process );
        ...
        else
        {
            ...
            ProductVersion lowerVersion = null;
            if ( lowerBound.length() > 0 )
            {
                lowerVersion = new PlatformProductVersion( lowerBound );
            }
            ProductVersion upperVersion = null;
            if ( upperBound.length() > 0 )
            {
                upperVersion = new PlatformProductVersion( upperBound );
            }
    ...
    public static VersionRange createFromVersion( String version ) {
        ...
        return new VersionRange( new PlatformProductVersion( version ), restrictions );
    }
...
*/
    ...
    private List<Restriction> intersection( List<Restriction> r1, List<Restriction> r2 )
    {
        ...
        while ( !done )
        {
                    ...
                    || res1.getLowerBound().compareTo( res2.getUpperBound() ) <= 0 )
            {
                ...
                {
                    ProductVersion lower;
                    ProductVersion upper;
    ...
    public ProductVersion getSelectedVersion( Product product )
            throws OverConstrainedVersionException
    {
        ...
        else
        {
            if ( restrictions.size() == 0 )
            {
                throw new OverConstrainedVersionException( "The product has no valid ranges", product );
            }
    ...
    public boolean isSelectedVersionKnown( Product product )
            throws OverConstrainedVersionException
    {
        boolean value = false;
        if ( recommendedVersion != null )
        ...
        else
        {
            if ( restrictions.size() == 0 )
            {
                throw new OverConstrainedVersionException( "The artifact has no valid ranges", product );
            }
        }
        return value;
    }
    ...
    public ProductVersion matchVersion( List<ProductVersion> versions )
    {
        // TODO: could be more efficient by sorting the list and then moving along the restrictions in order?

        ProductVersion matched = null;
